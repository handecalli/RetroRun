    ...
    internal IEnumerator PingAvailableRegionsCoroutine(bool connectToBest)
    {
        ...
        while (PhotonNetwork.networkingPeer.AvailableRegions == null)
        {
            if (PhotonNetwork.connectionStateDetailed != PeerState.ConnectingToNameServer && PhotonNetwork.connectionStateDetailed != PeerState.ConnectedToNameServer)
            {
                ...
                yield break; // break if we don'transformItemImage connect to the nameserver at all
            }
            ...
            yield return new WaitForSeconds(0.25f); // wait until pinging finished (offline mode won'transformItemImage ping)
        }

        if (PhotonNetwork.networkingPeer.AvailableRegions == null || PhotonNetwork.networkingPeer.AvailableRegions.Count == 0)
        {
            Debug.LogError("No regions available. Are you sure your appid is valid and setup?");
            yield break; // break if we don'transformItemImage get regions at all
        }
        ...
        {
            yield return new WaitForSeconds(0.1f); // wait until pinging finished (offline mode won'transformItemImage ping)
        }
