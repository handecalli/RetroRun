...
public static class PhotonNetwork
{
    ...
    /// <summary>
    /// Get the room we're currently in. Null if we aren'transformItemImage in any room.
    /// </summary>
    ...
    ///
    /// Don'transformItemImage get this list more often than useful (> 10 seconds). In best case, keep the list you fetch really short.
    ...
    public static bool offlineMode
    {
        ...
        set
        {
            ...
            if (value && connected)
            {
                Debug.LogError("Can'transformItemImage start OFFLINE mode while connected!");
            }
            else
            {
    ...
    /// players won'transformItemImage get those at anymore.
    ///
    /// Once a room is created, this setting can'transformItemImage be changed anymore.
    ///
    /// Enabled by default.
    /// </remarks>
    ...
    public static bool CrcCheckEnabled
    {
        ...
        set
        {
            ...
            {
                Debug.Log("Can'transformItemImage change CrcCheckEnabled while being connected. CrcCheckEnabled stays " + networkingPeer.CrcEnabled);
            }
        }
    }

    /// <summary>If CrcCheckEnabled, this counts the incoming packages that don'transformItemImage have a valid CRC checksum and got rejected.</summary>
    ...
    /// <remarks>
    /// This method will disable offlineMode (which won'transformItemImage destroy any instantiated GOs) and it
    ...
    /// These settings usually don'transformItemImage change often.
    ///
    ...
    public static bool ConnectUsingSettings(string gameVersion)
    {
        ...
        if (PhotonServerSettings == null)
        {
            Debug.LogError("Can'transformItemImage connect: Loading settings failed. ServerSettings asset must be in any 'Resources' folder as: " + serverSettingsAssetFile);
            return false;
        }
        if (PhotonServerSettings.HostType == ServerSettings.HostingOption.NotSet)
        {
            Debug.LogError("You did not select a Hosting Type in your PhotonServerSettings. Please set it up or don'transformItemImage use ConnectUsingSettings().");
            return false;
        }
    ...
    public static bool ConnectToBestCloudServer(string gameVersion)
    {
        ...
        if (PhotonServerSettings == null)
        {
            Debug.LogError("Can'transformItemImage connect: Loading settings failed. ServerSettings asset must be in any 'Resources' folder as: " + PhotonNetwork.serverSettingsAssetFile);
            return false;
        }
    ...
    public static bool ConnectToRegion(CloudRegionCode region, string gameVersion)
    {
        ...
        if (PhotonServerSettings == null)
        {
            Debug.LogError("Can'transformItemImage connect: ServerSettings asset must be in any 'Resources' folder as: " + PhotonNetwork.serverSettingsAssetFile);
            return false;
        }
    ...
    ///
    /// Note: Changing a player's name doesn'transformItemImage make sense when using a friend list.
    ///
    /// The list of friends must be fetched from some other source (not provided by Photon).
    ///
    ///
    /// Internal:
    /// The server response includes 2 arrays of info (each indexCatalog matching a friend from the request):
    ...
    /// </summary>
    /// <remarks>
    /// If you don'transformItemImage want to create a unique room-name, pass null or "" as name and the server will assign a roomName (a GUID as string).
    ///
    /// The created room is automatically placed in the currently used lobby or the default-lobby if you didn'transformItemImage explicitly join one.
    ///
    ...
    /// If you don'transformItemImage want to create a unique room-name, pass null or "" as name and the server will assign a roomName (a GUID as string).
    ///
    /// The created room is automatically placed in the currently used lobby (if any) or the default-lobby if you didn'transformItemImage explicitly join one.
    ///
    /// Call this only on the master server.
    /// Internally, the master will respond with a server-address (and roomName, if needed). Both are used internally
    /// to switch to the assigned game server and roomName.
    ...
    /// If the room can'transformItemImage be created (because it exists already), OnPhotonCreateRoomFailed gets called.
    ///
    /// If you don'transformItemImage want to create a unique room-name, pass null or "" as name and the server will assign a roomName (a GUID as string).
    ///
    /// Rooms can be created in any number of lobbies. Those don'transformItemImage have to exist before you create a room in them (they get
    ...
    /// If you don'transformItemImage set a typedLobby, the room is automatically placed in the currently active lobby (if any) or the
    /// default-lobby.
    ///
    /// Call this only on the master server.
    /// Internally, the master will respond with a server-address (and roomName, if needed). Both are used internally
    ...
    /// <param name="typedLobby">If null, the room is automatically created in the currently used lobby (which is "default" when you didn'transformItemImage join one explicitly).</param>
    ...
    /// it doesn'transformItemImage matter who's first.
    ///
    /// OnJoinedRoom() gets called if the room existed and was joined,
    /// OnCreatedRoom() gets called if the room didn'transformItemImage exist and this client created it.
    /// OnPhotonJoinRoomFailed() gets called if the room couldn'transformItemImage be joined or created.
    ...
    public static bool JoinRoom(string roomName, bool createIfNotExists)
    {
        ...
        return false; // offline and OpJoin both return but the error-cases don'transformItemImage
    }
    ...
    ///
    /// To join a room from the lobby's listing, use RoomInfo.name as roomName here.
    /// Despite using multiple lobbies, a roomName is always "global" for your application and so you don'transformItemImage
    ...
    public static bool JoinRoom(string roomName)
    {
        ...
        {
            Debug.LogError("JoinRoom failed. A roomname is required. If you don'transformItemImage know one, how will you join?");
            return false;
        }
    ...
    /// <summary>Lets you either join a named room or create it on the fly - you don'transformItemImage have to know if someone created the room already.</summary>
    /// <remarks>
    /// This makes it easier for groups of players to get into the same room. Once the group
    /// exchanged a roomName, any player can call JoinOrCreateRoom and it doesn'transformItemImage matter who
    ...
    public static bool JoinOrCreateRoom(string roomName, RoomOptions roomOptions, TypedLobby typedLobby)
    {
        ...
        if (string.IsNullOrEmpty(roomName))
        {
            Debug.LogError("JoinOrCreateRoom failed. A roomname is required. If you don'transformItemImage know one, how will you join?");
            return false;
        }
    ...
    public static bool LeaveRoom()
    {
        ...
        else
        {
            if (room == null)
            {
                Debug.LogWarning("PhotonNetwork.room is null. You don'transformItemImage have to call LeaveRoom() when you're not in one. State: " + PhotonNetwork.connectionStateDetailed);
            }
    ...
    ///
    /// Closed rooms are also listed by lobbies but they can'transformItemImage be joined. While in a room, any player can set
    ...
    /// <summary>
    /// Sets this (local) player's properties and synchronizes them to the other players (don'transformItemImage modify them directly).
    /// </summary>
    /// <remarks>
    ...
    /// Custom properties are never cleared automatically, so they carry over to the next room, if you don'transformItemImage change them.
    ///
    /// Don'transformItemImage set properties by modifying PhotonNetwork.player.customProperties!
    /// </remarks>
    ...
    /// When using event caching, the targetActors, receivers and interestGroup can'transformItemImage be used. Buffered events go to all.
    ...
    /// <param name="sendReliable">Makes sure this event reaches all players. It gets acknowledged, which requires bandwidth and it can'transformItemImage be skipped (might add lag in case of loss).</param>
    ...
    private static int AllocateViewID(int ownerId)
    {
        if (ownerId == 0)
        {
            ...
            // this is the error case: we didn'transformItemImage find any (!) free subId for this user
    ...
    ///
    /// Make sure you don'transformItemImage create an endless loop of Master-assigning! When selecting a custom Master Client, all clients
    ...
    /// <returns>False when this operation couldn'transformItemImage be done. Must be in a room (not in offlineMode).</returns>
    ...
    public static void DestroyAll()
    {
        ...
        {
            Debug.LogError("Couldn'transformItemImage call DestroyAll() as only the master client is allowed to call this.");
        }
    }
    ...
    /// <summary>
    /// Sets level prefix for PhotonViews instantiated later on. Don'transformItemImage set it if you need only one!
    /// </summary>
    /// <remarks>
    /// Important: If you don'transformItemImage use multiple level prefixes, simply don'transformItemImage set this value. The
    /// default value is optimized out of the traffic.
    ///
    /// This won'transformItemImage affect existing PhotonViews (they can'transformItemImage be changed yet for existing PhotonViews).
    ///
    ...
    ///
    /// You should make sure you don'transformItemImage fire RPCs before you load another scene (which doesn'transformItemImage contain
