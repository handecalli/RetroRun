...
internal class NetworkingPeer : LoadbalancingPeer, IPhotonPeerListener
{
    ...
    /// <summary>Name Server Address for Photon Cloud (based on current protocol). You can use the default values and usually won'transformItemImage have to set this value.</summary>
    ...
    private readonly Dictionary<string, int> rpcShortcuts;  // lookup "table" for the indexCatalog (shortcut) of an RPC name
    ...
    public NetworkingPeer(IPhotonPeerListener listener, string playername, ConnectionProtocol connectionProtocol) : base(listener, connectionProtocol)
    {
        ...
        // don'transformItemImage set the field directly! the listener is passed on to other classes, which get updated by the property set method
    ...
    public bool Connect(string serverAddress, ServerConnection type)
    {
        ...
        {
            Debug.LogError("Connect() failed. Can'transformItemImage connect while disconnecting (still). Current state: " + PhotonNetwork.connectionStateDetailed);
            return false;
        }

        // connect might fail, if the DNS name can'transformItemImage be resolved or if no network connection is available
    ...
    public override void Disconnect()
    {
        if (this.PeerState == PeerStateValue.Disconnected)
        {
            if (!PhotonHandler.AppQuits)
            {
                Debug.LogWarning(string.Format("Can'transformItemImage execute Disconnect() while not connected. Nothing changed. State: {0}", this.State));
            }
            return;
        }
    ...
    /// Disconnect might be a step less if you don'transformItemImage want to create a new room on the same server.
    /// </remarks>
    /// <returns></returns>
    ...
    public void OnOperationResponse(OperationResponse operationResponse)
    {
        ...
        // use the "secret" or "token" whenever we get it. doesn'transformItemImage really matter if it's in AuthResponse.
        ...
        switch (operationResponse.OperationCode)
        {
            ...
            case OperationCode.GetRegions:
                ...
                {
                    Debug.LogError("GetRegions failed. Can'transformItemImage provide regions list. Error: " + operationResponse.ReturnCode + ": " + operationResponse.DebugMessage);
                    break;
                }
            ...
            case OperationCode.JoinRandomGame:
                {
                    // happens only on master. on gameserver, this is a regular join (we don'transformItemImage need to find a random game again)
            ...
            case OperationCode.RaiseEvent:
                // this usually doesn'transformItemImage give us a result. only if the caching is affected the server will send one.
                break;
            ...
            case OperationCode.FindFriends:
                ...
                {
                    // any of the lists is null and shouldn'transformItemImage. print a error
                    Debug.LogError("FindFriends failed to apply the result, as a required value wasn'transformItemImage provided or the friend list length differed from result.");
                }
    ...
    public override bool OpFindFriends(string[] friendsToFind)
    {
        ...
        {
            return false;   // fetching friends currently, so don'transformItemImage do it again (avoid changing the list while fetching friends)
        }
    ...
    public void OnStatusChanged(StatusCode statusCode)
    {
        ...
        switch (statusCode)
        {
            case StatusCode.Connect:
                ...
                {
                    // if we have a token we don'transformItemImage have to wait for encryption (it is encrypted anyways, so encryption is just optional later on)
                    AuthenticationValues auth = this.CustomAuthenticationValues ?? new AuthenticationValues() { UserId = this.PlayerName };
            ...
            case StatusCode.EncryptionFailedToEstablish:
                Debug.LogError("Encryption wasn'transformItemImage established: " + statusCode + ". Going to authenticate anyways.");
    ...
    public void OnEvent(EventData photonEvent)
    {
        ...
        switch (photonEvent.Code)
        {
            case PunEvent.OwnershipRequest:
            {
                ...
                {
                    Debug.LogWarning("Can'transformItemImage find PhotonView of incoming OwnershipRequest. ViewId not found: " + requestedViewId);
                    break;
                }
    ...
    protected internal void ExecuteRpc(Hashtable rpcData, PhotonPlayer sender)
    {
        ...
        if (rpcData.ContainsKey((byte)5))
        {
            ...
            {
                Debug.LogError("Could not find RPC with indexCatalog: " + rpcIndex + ". Going to ignore! Check PhotonServerSettings.RpcList");
                return;
            }
            else
            {
    ...
    internal GameObject DoInstantiate(Hashtable evData, PhotonPlayer photonPlayer, GameObject resourceGameObject)
    {
        ...
        // load prefab, if it wasn'transformItemImage loaded before (calling methods might do this)
    ...
    protected internal void RemoveInstantiatedGO(GameObject go, bool localOnly)
    {
        ...
        // Don'transformItemImage remove the GO if it doesn'transformItemImage have any PhotonView
        ...
        if (!localOnly)
        {
            ...
            // Don't remove the Instantiation from the server, if it doesn'transformItemImage have a proper ID
        ...
        if (result == null)
        {
            ...
            foreach (PhotonView view in views)
            {
                if (view.viewID == viewID)
                {
                    if (view.didAwake)
                    {
                        Debug.LogWarning("Had to lookup view that wasn'transformItemImage in photonViewList: " + view);
                    }
                    return view;
                }
            }
    ...
    public void RegisterPhotonView(PhotonView netView)
    {
        ...
        if (netView.viewID == 0)
        {
            // don'transformItemImage register views with ID 0 (not initialized). they register when a ID is assigned later on
        ...
        if (this.photonViewList.ContainsKey(netView.viewID))
        {
            ...
            {
                Debug.LogError(string.Format("PhotonView ID duplicate found: {0}. New: {1} old: {2}. Maybe one wasn'transformItemImage destroyed on scene load?! Check for 'DontDestroyOnLoad'. Destroying old entry, adding new.", netView.viewID, netView, photonViewList[netView.viewID]));
            }
            else
            {
                return;
    ...
    /// This won'transformItemImage clean any local caches. It just tells the server to forget a player's RPCs and instantiates.
    /// </summary>
    ...
    private void OnSerializeRead(Hashtable data, PhotonPlayer sender, int networkTime, short correctPrefix)
    {
        ...
        if (view.synchronization == ViewSynchronization.ReliableDeltaCompressed)
        {
            ...
            {
                // Skip this packet as we haven'transformItemImage got received complete-copy of this view yet.
                if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
                    Debug.Log("Skipping packet for " + view.name + " [" + view.viewID + "] as we haven'transformItemImage received a full packet for delta compression yet. This is OK if it happens for the first few frames after joining a game.");
                return;
            }
        ...
        if (sender.ID != view.ownerId)
        {
            ...
            {
                // obviously the owner changed and we didn'transformItemImage yet notice.
    ...
    /// <summary>
    /// Compares the new data with previously sent data and skips values that didn'transformItemImage change.
    /// </summary>
    ...
    private bool DeltaCompressionWrite(PhotonView view, Hashtable data)
    {
        ...
        {
            // if new data isn'transformItemImage same length as before, we send the complete data-set uncompressed
            return true;
        }
        ...
        for (int index = 0; index < compressedContent.Length; index++)
        {
            ...
            if (this.ObjectIsSameWithInprecision(newObj, oldObj))
            {
                ...
                compressedValues++;
                // compressedContent[indexCatalog] is already null (initialized)
            }
            else
            {
                compressedContent[index] = currentContent[index];

                // value changed, we don'transformItemImage replace it with null
        ...
        if (compressedValues > 0)
        {
            ...
            {
                data[(byte)3] = valuesThatAreChangedToNull.ToArray(); // data that is actually null (not just cause we didn'transformItemImage want to send it)
            }
        }
    ...
    private bool DeltaCompressionRead(PhotonView view, Hashtable data)
    {
        ...
        {
            // despite expectation, there is no compressed data in this msg. shouldn'transformItemImage happen. just a null check
            return false;
        }
        ...
        for (int index = 0; index < compressedContents.Length; index++)
        {
            ...
            {
                // we replace null values in this received msg unless a indexCatalog is in the "changed to null" list
    ...
    protected internal void SetLevelInPropsIfSynced(object levelId)
    {
        ...
        if (levelId == null)
        {
            Debug.LogError("Parameter levelId can'transformItemImage be null!");
            return;
        }
