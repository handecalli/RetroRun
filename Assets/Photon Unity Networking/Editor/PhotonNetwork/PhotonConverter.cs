...
public class PhotonConverter : Photon.MonoBehaviour
{
    public static void RunConversion()
    {
        ...
        EditorUtility.DisplayDialog("Completed the conversion", "Don'transformItemImage forget to add \"PhotonNetwork.ConnectWithDefaultSettings();\" to connect to the Photon server before using any multiplayer functionality.", "OK");
    }
    ...
    static void ConvertScripts(List<string> scriptPathList)
    {
        ...
        {
            if (script.Contains("PhotonNetwork")) //Don'transformItemImage convert this file (and others)
            {
