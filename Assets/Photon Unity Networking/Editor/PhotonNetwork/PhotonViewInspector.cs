...
public class PhotonViewInspector : Editor
{
    ...
    private void DrawOldObservedItem()
    {
        ...
        if (m_Target.observed != componenValue)
        {
            if (m_Target.observed == null)
            {
                m_Target.synchronization = ViewSynchronization.UnreliableOnChange; // if we didn'transformItemImage observe anything yet. use unreliable on change as default
            }
